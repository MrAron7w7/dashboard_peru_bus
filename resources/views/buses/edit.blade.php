    ...
    <div class="max-w-3xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Modifica la información del bus
            </p>
        </div>

        <!-- Formulario -->
        <form method="POST" action="{{ route('buses.update', $bus) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="placa" id="placa" value="{{ old('placa', $bus->placa) }}"
            ...
            <div>
                ...
                <input type="text" name="modelo" id="modelo" value="{{ old('modelo', $bus->modelo) }}"
            ...
            <div>
                ...
                <input type="number" name="capacidad" id="capacidad" value="{{ old('capacidad', $bus->capacidad) }}"
                    class="mt-1 block w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm dark:text-white"
                    required>
                @error('capacidad')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <input type="text" step="0.01" name="estado" id="estado" value="{{ old('estado', $bus->estado) }}"
                    class="mt-1 block w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm dark:text-white"
                    required>
                @error('estado')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
